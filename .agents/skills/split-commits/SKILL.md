...
7. **Sanity check the plan**
   ...
   **Validate the order by mentally executing each commit:**
   ```
...
2. **For each planned commit:**
   ...
   d. **Run the repository's checks before committing:** the command its `AGENTS.md` or `package.json` names for lint, typecheck and tests (in theseus, `npm run check`). Don't assume a pre-commit hook runs any of them.
      Passing checks show the commit works on its own and expose dependencies on later commits.
...
4. **Audit each commit**
   ...
   <the repository's check command, as in step d>
   git checkout "$WORK_BRANCH"
   ```

   Each commit passes lint, typecheck and tests on its own.
...
When a split goes wrong (coupled changes that seem to force one big commit, checks failing mid-process, interleaved changes in one file, or a final state that doesn't match the original), read `references/troubleshooting.md`.
